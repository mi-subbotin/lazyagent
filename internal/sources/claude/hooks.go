...
// full path inside the JSON so a future writer (PRI-1 phase 2) can
// round-trip an edited entry back to its slot:
//
//   hooks/<event>/<matcher-idx>/<hook-idx>
//
	...
	for event, eventEntriesAny := range hooksAny {
		...
		for matcherIdx, eventEntryAny := range eventEntries {
			...
			for hookIdx, innerAny := range innerHooks {
				...
				configKey := fmt.Sprintf("hooks/%s/%d/%d", event, matcherIdx, hookIdx)
